    ...
    def run(self) -> str:
        ...
        if status == 200 and body:
            if self._is_catch_all(body):
                self._catch_all = True
                print(f"\n  {'⚠️':<4} ⚠️  CATCH-ALL DETECTED — server returns homepage for '{login_path}'")
                print(f"  {'':<4} This endpoint likely doesn't exist. Skipping rate limiting and lockout tests.")
                print(f"  {'':<4} Results would be FALSE POSITIVES.\n")
        ...
        print(f"  🔍 1/3 — Rate Limiting Test")
        if self._catch_all:
            print(f"    {'⏭️':<4} Skipped — login endpoint doesn't exist (catch-all routing)")
            self.report.add_finding("low", "Rate limiting test skipped (catch-all routing)",
                detail="Login endpoint returned homepage instead of 404. Cannot test rate limiting on non-existent endpoint.")
        else:
            requests_made = 0
            rate_limited = False
            for i in range(15):
        ...
        if self._catch_all:
            print(f"    {'⏭️':<4} Skipped — login endpoint doesn't exist (catch-all routing)")
            self.report.add_finding("low", "Lockout test skipped (catch-all routing)",
                detail="Login endpoint returned homepage instead of 404. Cannot test lockout on non-existent endpoint.")
        else:
            locked_out = False
            ...
            if locked_out:
                ...
                self.report.add_finding("critical", "No lockout detected",
                    detail=f"All {RAPID_COUNT} rapid requests succeeded",
